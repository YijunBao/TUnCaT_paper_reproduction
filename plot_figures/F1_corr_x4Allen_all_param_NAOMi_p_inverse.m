...
for vid = 1:num_video
    ...
    gap = 0.02;  marg_h = 0.07; marg_w = 0.04; 
    ...
    for ind_param_vary = 1:num_param_vary
        ...
        %% 
        load(['NAOMi\timing_x5_',param_vary,'_GCaMP6f_all_methods_split ',addon,baseline_std,'.mat']);
...
%         ylim(yl2);
    ...
    %     ylim(yl2);
        ...
        corr_all = squeeze(corr_CV(:,:,:)); % thred_ratio_CV 
        corr = squeeze(mean(corr_all,1))';
        corr_err = squeeze(std(corr_all,1,1))';

        axes(ha(sub2ind([num_param_vary, 3], ind_param_vary,2)));
        ax(2,ind_param_vary) = gca;
        hold on;
        h=plot(list_T',corr,'.-','MarkerSize',18,'LineWidth',2);
        h(end).Color = color(5,:);
        for ind = 1:num_method
            errorbar(list_T',corr(:,ind),corr_err(:,ind),corr_err(:,ind),...
                'LineWidth',1,'Color',h(ind).Color,'HandleVisibility','off');
        end
%         yl2 = [0,0];
%         yl2(1) = floor(min(min(corr-corr_err))/grid_score)*grid_score;
%         yl2(2) = ceil(max(max(corr+corr_err))/grid_score)*grid_score;
%         step = (yl2(2)-yl2(1))*ratio_step;
%         step_ns = step*ratio_ns;
%         list_y = max(max(corr+corr_err))+step*(1:num_method-1);
%         yl2(2) = ceil(list_y(end)/grid_score)*grid_score;
%         ylim(yl2);
    %     yl=get(gca,'Ylim');
    %     list_y_corr = max(max(corr+corr_err))+step_linear*(1:num_method);
    %     yl2 = [floor(yl(1)/0.1)*0.1,1.2];
    %     ylim(yl2);
        for ind = 1:num_method
            if ind < num_method
                for jj = 1:num_param
                    p_sign = signrank(corr_all(:,ind,jj),corr_all(:,end,jj)); % 
                    p_range = find(p_sign > list_p_cutoff,1,'first');
                    if p_range == 1
                        y = list_y(ind)+step_ns;
                    else
...
%         legend(list_method,'Location','NorthOutside','NumColumns',2) % 'FontSize',12,
    ...
    set(ax(2,:),'XTickLabel',{},'YLim',yl_corr);
    set(ax(3,:),'YLim',yl_time);
    set(ax(:,6),'XTick',1:6);
    set(ax(:,2:num_param_vary),'YTickLabel',{});
    set(ax(3,end),'XTick',list_param,'XTickLabel',cellfun(@(x) x(end-1:end), list_prot, 'UniformOutput',false));
...
%     end
    saveas(gcf,['NAOMi_x4 vary all params ',video,' videos p inverse log 0908.png']);
