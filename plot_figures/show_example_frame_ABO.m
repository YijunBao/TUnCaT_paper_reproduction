...
% toc;
load([dir_video,'traces_ours_Raw_novideounmix_r2\raw\',Exp_ID,'.mat'],'traces','bgtraces');
traces = traces';
bgtraces = bgtraces';

%%
nn=34; % 16 for 10, 51 for 10, 55 for 4
mask=ROIs(:,:,nn);
...
% figure; imshow3D(video_show,imshow_range,1853);
...
%%
% imshow_range = [150,450]; % [300,600]; % [250,550]; % 
list_t = [5205, 14622, 15335, 19872];
% list_t = [1849, 2582, 8592, 10316, 19232, 19782, 22376]; % 34 of 2 SNR
% list_t = [1853, 2584, 8594, 10317, 19234, 19783, 22378]; % 34 of 2 raw
% list_t = [147, 4154, 7069 10228, 12854, 16736, 17934]; % 22 of 3
% list_t = [5444, 8670, 16094, 16337, 19851, 22281, 22375]; % 55 of 4
% list_t = [5445, 8671, 16095, 16339, 19852, 22284, 22375]; % 55 of 4 better display
% list_t=[1251, 3577, 12293, 14776, 17678, 19384, 20357]; % 27 of 10
% list_t=[10313,11004,11793,12106]; % 16 of 10
% list_t=[7934,8124,8225,10422]; % 51 of 10
% list_t=[7934];
for t_input = list_t
...
set(h,'FontSize',14);
hold on; contour(mask_show,'LineColor',blue);
...
contour(kron(circle(xmin_show:xmax_show,ymin_show:ymax_show),logical(mag_kernel)),'LineColor',purple)
