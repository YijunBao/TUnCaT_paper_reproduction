...
%% ABO or 1p
spike_type = 'include'; % {'exclude','only','include'};
...
load([spike_type,'\timing_split_BinUnmix',addon,'_100.mat'],...
...
ylim(ylim_set);
saveas(gcf,[spike_type,' F1 bin downsample p 0910.png']);
saveas(gcf,[spike_type,' F1 bin downsample p 0910.emf']);
...
%     suptitle(title_str);
% ylim([15,50]);
saveas(gcf,[spike_type,' time bin downsample p 0910.png']);
saveas(gcf,[spike_type,' time bin downsample p 0910.emf']);
...
%     title_str = [video,' NAOMi videos with different frequency'];
%     suptitle(title_str);
    saveas(gcf,[spike_type,' alpha bin downsample 0810.png']);
end
...
%         bin_option = num_bin_option(oid);
    ...
    list_precision = list_precision_all{bid,oid,vid};
    list_F1 = list_F1_all{bid,oid,vid};
    list_corr_unmix = list_corr_unmix_all{bid,oid,vid};
    ...
    for CV = 1:num_Exp
        ...
        if min(n2,n3)>1
            ...
            corr_CV(CV,bid,oid,vid) = list_corr_unmix(CV,ind_corr);
            Table_recall_CV{bid,oid,vid}(CV,:) = permute(list_recall(CV,ind_alpha,:),[1,3,2]);
            Table_precision_CV{bid,oid,vid}(CV,:) = permute(list_precision(CV,ind_alpha,:),[1,3,2]);
            Table_F1_CV{bid,oid,vid}(CV,:) = permute(list_F1(CV,ind_alpha,:),[1,3,2]);
        else
            ...
            time_CV(CV,bid,oid,vid) = Table_time(CV);
            corr_CV(CV,mid,vid,pid) = list_corr_unmix(CV);
        end
    end
    end
end
...
% ylim([0.65,1]);
saveas(gcf,[spike_type,' F1 bin downsample p 0810.png']);
saveas(gcf,[spike_type,' F1 bin downsample p 0810.emf']);

figure('Position',[1000,100,450,450]);
hold on;
for vid = 1:num_video
    F1_all = squeeze(corr_CV(:,:,:,vid));
    F1 = squeeze(mean(F1_all,1))';
    F1_err = squeeze(std(F1_all,1,1))';
    h=plot(list_nbin,F1,'.-','MarkerSize',18,'LineWidth',2,'Color',color(5+vid,:));
    errorbar(list_nbin,F1,F1_err,F1_err,...
...
saveas(gcf,[spike_type,' corr bin downsample p 0810.png']);
saveas(gcf,[spike_type,' corr bin downsample p 0810.emf']);
...
%     title_str = [video,' NAOMi videos with different frequency'];
%     suptitle(title_str);
saveas(gcf,[spike_type,' time bin downsample videos p 0810.png']);
saveas(gcf,[spike_type,' time bin downsample videos p 0810.emf']);
...
%     suptitle(title_str);
    saveas(gcf,[spike_type,' alpha bin downsample 0810.png']);
    saveas(gcf,[spike_type,' alpha bin downsample 0810.emf']);
end
