...
%         folder = sprintf('traces_ours');
        ...
        for ii = 1:num_Exp
            ...
            C_gt = C_gt(select_SUNS,:);
            YrA_gt = YrA_gt(select_SUNS,:);
            traces_pure = C_gt;
            noise_pure = YrA_gt;
...
%             fprintf('\b\b\b\b\b\b\b\b\b\b\b\b\b\b');
        ...
        save(sprintf('%s\\scores_split_%s_%sVideo%s%s_%s_%s%s_common.mat',spike_type,method,video,addon,ndiag,num2str(ThJaccard),sigma_from,baseline_std),...
