...
method = 'SUNS_complete+CNMF'; % {'FISSA','ours','CNMF'}
list_video={'Raw','SNR'};
% video='Raw'; % {'Raw','SNR'}
addon = '';
...
%             load(fullfile(dir_FISSA,[Exp_ID,'.mat']),'C_gt','YrA_gt');
            ...
            load(fullfile(dir_FISSA,[Exp_ID,'.mat']),'C_gt','YrA_gt');
            if onlyTP
                C_gt = C_gt(select_seg,:);
                YrA_gt = YrA_gt(select_seg,:);
            else
                T = size(C_gt,2);
                C=zeros(nGT,T);
                YrA = zeros(nGT,T);
                C(select_GT,:) = C_gt(select_seg,:);
                YrA(select_GT,:) = YrA_gt(select_seg,:);
                C = [C;C_gt(remaining,:)];
                YrA = [YrA;YrA_gt(remaining,:)];
                C_gt = C;
                YrA_gt = YrA;
            end
            traces_pure = C_gt;
...
%             fprintf('\b\b\b\b\b\b\b\b\b\b\b\b\b\b');
        ...
        if onlyTP
            save(sprintf('%s\\scores_split_%s_%sVideo%s%s_%s_%s%s_onlyTP.mat',spike_type,method,video,addon,ndiag,num2str(ThJaccard),sigma_from,baseline_std),...
                'list_recall','list_precision','list_F1','list_thred_ratio');
        else
            save(sprintf('%s\\scores_split_%s_%sVideo%s%s_%s_%s%s_hasFNFP.mat',spike_type,method,video,addon,ndiag,num2str(ThJaccard),sigma_from,baseline_std),...
                'list_recall','list_precision','list_F1','list_thred_ratio');
        end
