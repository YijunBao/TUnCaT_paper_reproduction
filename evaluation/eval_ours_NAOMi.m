...
addon = '_novideounmix_r2_mixout'; % _fixed_alpha_mixout _n_iter '_pertmin=0.16_eps=0.1_range'; %  _direction_AllenSubs0 
...
folder = sprintf('traces_%s_%s%s%s%s%s',method,'SNR',list_part1{1},list_part2{1},list_part3{1},addon);
...
%                         list_thred_ratio=10:10:110; % GCaMP6s;
                    else
...
%             saveas(gcf,sprintf('simulation\\%s Our unmixing, %s Video %s_comp Sigma.png',simu_opt,video,sigma_from));
            ...
            disp([list_alpha(ind1), list_thred_ratio(ind2),max_F1,max(mean_corr)])
            figure; plot(list_alpha,mean_corr,'LineWidth',2); 
            set(gca,'XScale','log'); xlabel('\alpha');
            fprintf('\b');
            if ind1 == 1
                disp('Decrease alpha');
            elseif ind1 == L1
...
%             save(sprintf('simulation\\scores_%s_%s_%sVideo_%s_compSigma_BinUnmix%s%d.mat',method,simu_opt,video,sigma_from,bin_option,nbin),...
            save(sprintf('NAOMi\\scores_split_%s_%s_%sVideo_%s_compSigma%s%s%s%s%s.mat',method,simu_opt,video,sigma_from,part1,part2,part3,addon,baseline_std),...
