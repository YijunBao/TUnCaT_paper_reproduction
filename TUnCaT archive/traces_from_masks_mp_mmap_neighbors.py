...
def mean_median_out_trace(nn, fp_video, video_shape, video_dtype, \
    fp_masks, fp_xx, fp_yy, r_bg, comx, comy, area, neighbors):
    mask = fp_masks[nn]
...
def traces_bgtraces_from_masks_neighbors(fp_video, video_dtype, video_shape, \
        fp_masks, masks_shape, name_mmap):
    ...
    results = p.starmap(mean_median_out_trace, [(nn, fp_video, video_shape, video_dtype, \
        fp_masks, fp_xx, fp_yy, r_bg, comx, comy, area[nn], list_neighbors[nn]) for nn in range(ncells)], chunksize=1)
    p.close()
