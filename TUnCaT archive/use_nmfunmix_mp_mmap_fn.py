...
from nmfunmix1_mmap_fn import nmfunmix1


def use_nmfunmix(trace, bgtraces, masks_shape, fn_video, video_dtype, \
        fn_masks, fn_xx, fn_yy, comx, comy, r_bg, alpha, nbin=1):
    ...
    results = p.starmap(nmfunmix1, [(i, trace, bgtraces[:,i], fn_video, (Lx,Ly), video_dtype, fn_masks, \
        fn_xx, fn_yy, alpha, comx, comy, r_bg, nbin, bin_option) for i in range(n)], chunksize=1)
    p.close()
